...
export async function safeRead(page, fn, arg) {
    try {
 ...
 */
export async function settle(page, budgetMs = 1500) {
...
export async function detectSession(page, ctx = {}) {
    const checked = [];
    ...
    const emptyBody = await safeRead(page, () => document.readyState === "complete" &&
        !!document.body &&
    ...
    const captcha = await safeRead(page, () => {
        ...
        const frame = iframes.find((f) => rx.test(f.src));
        if (frame)
            return "iframe:" + frame.src.slice(0, 80);
        const title = document.title || "";
        if (title.slice(0, 13) === "Just a moment")
            return "cloudflare-title";
        if (document.querySelector('#challenge-running, [id^="cf-chl"]'))
    ...
    const login = await safeRead(page, () => {
        ...
        for (let i = 0; i < pws.length; i++) {
            const e = pws[i];
            const vis = typeof e.checkVisibility === "function"
                ? e.checkVisibility()
    ...
    const overlay = await safeRead(page, (pt) => {
        const p = pt;
        ...
        if (modal) {
            ...
            if (covers || overPoint)
                return "modal:" + modal.tagName.toLowerCase();
        }
        ...
        while (el && el !== document.body) {
            ...
            if ((cs.position === "fixed" || cs.position === "sticky") && cs.pointerEvents !== "none") {
                ...
                if (r.width * r.height >= 0.8 * area) {
                    return "overlay:" + (el.id || el.className || el.tagName);
                }
            }
            el = el.parentElement;
        }
