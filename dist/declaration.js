...
import { pollUntil, readTarget, readTree, redactLen, treeText, } from "./postcondition.js";
const isVacuous = (m) => (typeof m === "string" ? m === "" : m.test(""));
...
export function validateDeclarations(input) {
    ...
    for (const d of list) {
        const bad = (why) => new Error(`TrueFact: vacuous declaration ${JSON.stringify(d, (_, v) => (v instanceof RegExp ? String(v) : v))} — ${why}`);
        switch (d.kind) {
            case "url":
                if (isVacuous(d.matches))
                    throw bad("url.matches would match any URL");
                break;
            case "text":
                if (isVacuous(d.matches))
                    throw bad("text.matches would match any page");
                break;
            case "element":
                if (!d.selector.trim())
                    throw bad("element.selector is empty");
                break;
            case "field":
                if (!d.selector.trim())
                    throw bad("field.selector is empty");
                break; // equals "" is allowed: it declares a cleared field
            case "probe":
                if (!d.get.trim())
                    throw bad("probe.get is empty");
                if (d.status === undefined && d.text === undefined)
                    throw bad("probe needs a status or text to match — a bare get asserts nothing");
                if (d.text !== undefined && isVacuous(d.text))
                    throw bad("probe.text would match any body");
                break;
        }
    }
    return list;
}
const matches = (m, s) => (typeof m === "string" ? s.includes(m) : m.test(s));
async function checkOne(page, d, tree) {
    switch (d.kind) {
        case "url": {
            let href;
            try {
        ...
        case "text": {
            if (d.role) {
                ...
                for (let i = 0; i < lines.length && hit === null; i++) {
                    if (!roleRx.test(lines[i]))
                        continue;
                    const parts = [];
                    for (let j = i; j < Math.min(lines.length, i + 6); j++)
                        parts.push(treeText(lines[j]));
                    const spaced = parts.filter(Boolean).join(" ");
                    const tight = parts.join("");
                    if (matches(d.matches, spaced))
                        hit = spaced;
                    else if (matches(d.matches, tight))
                        hit = tight;
                }
        ...
        case "probe": {
            ...
            if (status >= 500 && !expectedThis5xx)
                return { met: null, actual: `HTTP ${status}`, isPassword: false };
            let ok = true;
            if (d.status !== undefined)
                ok = d.status === "ok" ? res.ok : status === d.status;
 ...
 */
export async function checkDeclarations(page, decls, budgetMs) {
    ...
    const evaluate = async () => {
        ...
        for (const d of decls) {
            ...
            const declaration = r.isPassword && d.kind === "field" && typeof d.equals === "string" ? { ...d, equals: redactLen(d.equals) } : d;
            out.push({
                declaration,
                met: r.met,
...
const LIFTABLE = new Set(["no-change", "changed-unclassified", "hash-only-nav", "unsettled", "error-text"]);
/**
 ...
 */
export function applyDeclarations(auto, results) {
    if (results.length === 0)
        return auto;
    if (results.some((r) => r.met === null))
        return { verdict: "inconclusive", reason: "declared-unreadable", confidence: "heuristic" };
    if (results.some((r) => r.met === false))
        return { verdict: "did-not-land", reason: "declared-unmet", confidence: "high" };
    const positives = results.filter((r) => !("absent" in r.declaration && r.declaration.absent));
    if (positives.length === 0)
        return auto;
    if (auto.verdict === "landed")
        return { ...auto, confidence: "high" };
    if (LIFTABLE.has(auto.reason))
        return { verdict: "landed", reason: "declared-met", confidence: "high" };
    return auto;
}
