...
export async function runDemo() {
    ...
    let Stagehand;
    try {
        ({ localBrowser, Stagehand } = (await import("@browserbasehq/stagehand")));
    }
    catch {
        process.stderr.write("truefact demo needs a browser engine. Install the peer:\n" +
    ...
    await new Promise((r) => fx.listen(fxPort, "127.0.0.1", () => r()));
    ...
    const w = withTrueFact(playwrightDriver(page), { network: { port }, screenshots: false, waitMs: 1500, jsonl });
