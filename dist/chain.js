...
export function makeSigner(privateKeyPem) {
...
export function verifyHashSig(hashHex, sigB64, publicKeyPem) {
    try {
 ...
 */
export function verifyChain(steps, opts = {}) {
    let prev = "";
    for (let i = 0; i < steps.length; i++) {
        const s = steps[i];
        if ((s.prevHash ?? "") !== prev)
            return { ok: false, length: steps.length, brokenAt: i, reason: "prevHash does not link to the previous step (reordered or dropped)" };
        if (s.hash !== hashStep(s))
            return { ok: false, length: steps.length, brokenAt: i, reason: "hash mismatch (record was altered)" };
        if (opts.publicKey) {
            if (!s.sig)
                return { ok: false, length: steps.length, brokenAt: i, reason: "missing signature (a public key was given but this step is unsigned)" };
            if (!verifyHashSig(s.hash, s.sig, opts.publicKey))
                return { ok: false, length: steps.length, brokenAt: i, reason: "bad signature (wrong key or altered)" };
        }
        prev = s.hash;
    }
