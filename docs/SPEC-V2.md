# TrueReplay v2 — Ground truth for agent actions
...
Every tool that monitors agents reads the agent's own trace and grades it, usually with another model. TrueReplay reads the **world** the agent acted on and records it independently. The record serves three jobs, in order:
...
```ts
import { defineAssertions } from "truereplay";
...
```
truereplay assert runs/2026-09-17.jsonl --with assertions.ts
```
...
```
truereplay resume runs/x.jsonl --from 44
```
...
One static HTML file that loads a JSONL. Left: intent, verdict, assertion results per packet. Right: screenshot if present, else rendered a11y tree diff (before → after), plus the network list for that step. Scrub, click a step, see why. Served by `truereplay view run.jsonl` (opens the file). No server, no accounts, no build step. This is deliberately a page, not a product; viewers are commodity.
...
- **Canary halt:** `withReplay({ halt: { window: 50, maxDidNotLandRate: 0.05, onTrip } })` — trips a caller-supplied callback (pause the queue, page someone). TrueReplay never stops the fleet itself; it fires the signal.
- **Compare:** group verdicts by `agentVersion` tag; one table. This is the eval angle for free.

## 8. Ledger (the harvest, twenty lines)

- `hash` and `prevHash` on every packet from day one (§2). `truereplay verify run.jsonl` recomputes the chain and reports the first break.
- `sig` when `TRUEREPLAY_SIGNING_KEY` (ed25519, `node:crypto`) is set. Public key published by the operator.
...
| M1 | ✅ **DONE 2026-09-17 (chain slice)** — tamper-evident hash chain over the existing JSONL record: `record()` sets `prevHash`/`hash` (sha256 of the canonical step incl. the sealed agent_claim, minus hash/sig) per step; `verifyChain` (pure) + `truereplay verify <run.jsonl>` exit 1 at the first break. `src/chain.ts`, 178 tests incl. a real-run chain check. **Deferred:** the Step→Packet rename and the `boundary`/`actor` restructure (§2/§12) — pulled forward at M8 when a 2nd boundary needs the unified shape; `sig` is M9. The claim is already structurally sealed (no verdict/assertion path reads `agent_claim`; `viewOf` never touches it). | — | 1 d |
...
| M9 | Signing + `verify` + sinks | M1 | 0.5 d |
...
- **Observe vs gate (§4/§7).** `withReplay({ mode: "observe" })` records without polling; verdicts computed offline by `truereplay assert`. Gate mode (today's inline poll) stays for declared writes. Long runs need observe.
...
- **Learned postconditions (§4).** After N `landed` runs sharing an intent, propose an `expect` from tree lines present after-but-not-before with high frequency. Ships as `truereplay suggest run.jsonl`. Zero-config → precision ramp; only possible with ground-truth verdicts.
...
- **OpenTelemetry export.** Packets as spans with `truereplay.verdict` attributes → Raindrop/Sentrial/Datadog ingest us as the ground-truth feed.
