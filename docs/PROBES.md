...
when the write didn't land, and does TrueFact catch it?** Each run compares
...
Stagehand, no TrueFact — a pure thesis test.
...
oMLX on Apple Silicon (no cloud key), **wrapped by TrueFact**. Instruction: "click
...
2. TRUEFACT   : did-not-land  (no-change; session=overlay)
...
- **TrueFact verdict:** `did-not-land`, reason `no-change`, session obstruction
...
- **Result: false success, caught.** The click landed on the overlay and did nothing;
  Stagehand said success anyway; TrueFact flagged it. This is R2 working on a real
