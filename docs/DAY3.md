...
Detect the switch by the stable per-tab **`pageId`**, never by URL and never by object identity: `context.activePage()` returns a **fresh `Page` wrapper on every call** (verified during the build — `activePage() !== activePage()`), while `pageId` is stable across calls and survives navigation. Comparing object identity marks every write as a new tab. URL is no good either: over HTTP a new tab reports its URL within 100 ms, but a new tab opened to a `data:` URL reports `url() === ""` indefinitely (probe (b)) and would read as `blank`. That quirk is confined to `data:` tabs, which only exist in tests — one reason the tests use an HTTP fixture server (§9).
