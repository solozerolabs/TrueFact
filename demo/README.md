...
`truefact.gif` (embedded in the top-level README) is a two-scene cut, generated
...
ffmpeg -y -i demo/truefact.mp4 -vf "fps=15,scale=900:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=128[p];[s1][p]paletteuse=dither=bayer" demo/truefact.gif
```
