...
export interface Grounding {
  ...
  skipped: number; // leaves not judged: booleans, null, <3 or >120 chars, past the cap
  visual?: true; // extract ran with screenshot: true — the model may have read pixels
}

const MAX_LEAVES = 50;
const isNumericStr = (s: string) => /\d/.test(s) && /^[\s\d.,:/%$€£₹+-]+$/.test(s);
const numCore = (s: string) => s.replace(/,/g, "").replace(/^[^\d]+/, "").replace(/[^\d]+$/, "");
...
function groundNumber(raw: string, blobNoCommas: string): boolean {
  const v = numCore(raw);
  if (!v) return false;
  for (let from = 0; ; ) {
    ...
    if (!/\d/.test(l) && l !== "." && !/\d/.test(r)) return true;
    from = i + 1;
  }
}
...
function* leaves(data: unknown): Generator<unknown> {
  if (Array.isArray(data)) {
    for (const x of data) yield* leaves(x);
  } else if (data !== null && typeof data === "object") {
    for (const v of Object.values(data as Record<string, unknown>)) yield* leaves(v);
  } else {
    yield data;
  }
}
...
export function groundValues(data: unknown, tree: string[]): Grounding {
  ...
  const confidence: Confidence = values.every((v) => v.match === "exact") ? "high" : "heuristic";
