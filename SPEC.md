...
- **Day 6 — The benchmark** (the point of the whole week). Full spec: [docs/DAY6.md](docs/DAY6.md). Three channels, not two — the agent's claim and TrueReplay's verdict (both already in the replay) plus the fixture server's own state (the oracle), joined only by a pure scorer ([src/bench.ts](src/bench.ts)), never by the wrapper. The headline is a **MISS rate**: agent false-success `R = P(oracle=fail | claim=success)` versus TrueReplay's residual `M = P(verdict=landed | claim=success ∧ oracle=fail)`, both with a Wilson 95% interval over ≥200 decisive write steps; the product is real iff `M ≪ R`. **Two pre-registered gates ship together** — Gate A (the market exists: the *weakest* ladder rung silently fails ≥2%, since the long tail is the market) and Gate B (the instrument works: miss ≤2% **and** cry-wolf ≤1%); publish Day 7 iff both. The suite includes the adversarial `optimistic-ui` fixture (confirmation shown, server POST fails) so the miss rate is not rigged toward zero. Stagehand only, 5+ runs per task, agent claim and page truth on separate channels, four buckets counted at step level (headline) and run level. Three constraints the first draft missed:
