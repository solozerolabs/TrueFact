...
- **Day 4 — Postcondition: declared overrides.** Data-only declarations (`url`, `element`, `text`, `field`, plus `absent`) riding on `act(instruction, { expect })`; unmet → `did-not-land`, met lifts only the auto default's uncertain outcomes, never a mechanism-backed failure; one 5 s poll budget shared with the auto path; a run-level `finalize({ expect })` that can only demote. Folds in nine Day 3 revisions, the two that matter most being: a mixed fill+click no longer short-circuits on the field check, and bare `no-change` is `inconclusive` unless corroborated by an obstruction. Benchmark: `auto` and `declared` are separate columns; the kill signal is on `auto` only. Full spec: [docs/DAY4.md](docs/DAY4.md). *Done when:* an engineer declares on one write and gets a stricter verdict; a vacuous declaration is rejected at call time; un-declared writes still get the auto default, unchanged.
